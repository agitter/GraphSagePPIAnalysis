#!/usr/bin/env python3
"""Create a small provenance inventory for files that may be uploaded in batches.

The script never opens archives beyond reading their bytes for SHA-256. It does not
modify the source files. The resulting CSV is intended to be uploaded before any
large files so that upload batches can be planned without consuming attachment
storage.
"""
    ...
    for root in roots:
        ...
        for candidate in candidates:
            ...
            if resolved in seen:
                continue
            seen.add(resolved)
            rel = candidate.name if resolved_root.is_file() else str(candidate.relative_to(resolved_root))
            target_values = (candidate.name, rel)
            if include_patterns and not any(matches_any(v, include_patterns) for v in target_values):
                continue
            if exclude_patterns and any(matches_any(v, exclude_patterns) for v in target_values):
                continue
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Hash local files and write a provenance inventory CSV for staged uploads."
    )
    parser.add_argument(
        "paths",
        nargs="+",
    ...
    parser.add_argument(
        ...
        help="Output CSV path (default: local_upload_inventory.csv).",
    )
    parser.add_argument(
        "--patterns-file",
        type=Path,
        help="Optional text file containing filename/glob patterns to include, one per line.",
    )
    parser.add_argument(
        "--include",
        action="append",
...
def main() -> int:
    ...
    rows.sort(key=lambda row: (row["artifact_name"].lower(), row["relative_path"].lower()))
