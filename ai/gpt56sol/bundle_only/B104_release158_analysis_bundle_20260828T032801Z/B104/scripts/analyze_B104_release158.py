...
# 10. Exhaustive evidence masks + source/date sensitivity on selected terms
codes=sorted({x[2] for x in anns159}); sources=sorted({x[5] for x in anns159})
te={g:{e:0 for e in codes} for g in set(selected)}; tse={g:collections.defaultdict(int) for g in set(selected)}; tdate={g:collections.defaultdict(int) for g in set(selected)}
for a,direct,e,date,asp,assigned,ref,wf in anns159:
    gs=M159.get(a,set())
    if not gs: continue
    targets=set(selected).intersection(ancestors(direct))
    ...
    for gene in gs: b |= 1<<idx[gene]
    for q in targets:
        te[q][e] |= b; tse[q][(assigned,e)] |= b
        if e in BEST_EVIDENCE: tdate[q][date] |= b

def eval_pred(pred,cols=None):
    cols=list(range(121)) if cols is None else cols; total=fp=fn=exact=ge99=ge95=0
    for j in cols:
        p=pred.get(selected[j],0); o=labels[j]; d=(p^o).bit_count(); total+=d; fp+=(p&~o&ALL).bit_count(); fn+=(o&~p&ALL).bit_count(); exact+=d==0; ge99+=1-d/len(genes)>=.99; ge95+=1-d/len(genes)>=.95
    return {'total_mismatches':total,'false_positives':fp,'false_negatives':fn,'exact':exact,'at_least_99pct':ge99,'at_least_95pct':ge95}
def pred_mask(mc):
    return {g:functools.reduce(int.__or__,(te[g][e] for e in mc),0) for g in set(selected)}
ns_by_col=[terms.get(g,{}).get('namespace','') for g in selected]
mask_results={}
for scope,cols in [('all',list(range(121))),('BP',[i for i,n in enumerate(ns_by_col) if n=='biological_process']),('CC',[i for i,n in enumerate(ns_by_col) if n=='cellular_component']),('MF',[i for i,n in enumerate(ns_by_col) if n=='molecular_function'])]:
    best=[]
    for mask in range(1,1<<len(codes)):
        mc=[codes[k] for k in range(len(codes)) if mask>>k&1]; ev=eval_pred(pred_mask(mc),cols); key=(ev['total_mismatches'],ev['false_negatives'],ev['false_positives'],len(mc),','.join(mc))
        if len(best)<20 or key<best[-1][0]: best.append((key,mc,ev)); best.sort(key=lambda x:x[0]); best=best[:20]
    mask_results[scope]=[{'evidence_codes':','.join(x[1]),**x[2]} for x in best]
(ROOT/f'B104_evidence_mask_results_{STAMP}.json').write_text(json.dumps(mask_results,indent=2),encoding='utf-8')
basepred=pred_mask(BEST_EVIDENCE); base_eval=eval_pred(basepred)
source_rows=[]
for s in sources:
    p={g:0 for g in set(selected)}
    for g in set(selected):
        for ss in sources:
            if ss==s: continue
            for e in BEST_EVIDENCE: p[g] |= tse[g].get((ss,e),0)
    ev=eval_pred(p); source_rows.append({'excluded_source':s,**ev,'delta_mismatches':ev['total_mismatches']-base_eval['total_mismatches']})
source_rows.sort(key=lambda r:(r['total_mismatches'],r['false_negatives'],r['false_positives']))
write_csv(ANA/f'B104_source_leave_one_out_{STAMP}.csv',source_rows)
cur={g:0 for g in set(selected)}; date_rows=[]
for dt in sorted({d for q in tdate for d in tdate[q]}):
    for g in cur: cur[g] |= tdate[g].get(dt,0)
    ev=eval_pred(cur); date_rows.append({'cutoff_date':dt,**ev,'delta_mismatches':ev['total_mismatches']-base_eval['total_mismatches']})
