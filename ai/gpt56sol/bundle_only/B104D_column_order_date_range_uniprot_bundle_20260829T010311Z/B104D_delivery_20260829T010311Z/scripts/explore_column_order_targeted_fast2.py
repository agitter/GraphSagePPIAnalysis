...
OUT=ROOT/'analysis_deep'; OUT.mkdir(parents=True,exist_ok=True)
...
# 1. Static ontology / representation models.
static_sequences={
 'ontology_obo_primary':term_ids,
 'ontology_obo_primary_plus_alt':[x for g in term_ids for x in ([g]+alt_by_primary.get(g,[]))],
 'ontology_sorted_primary':sorted(term_ids),
 'ontology_name_sorted':sorted(term_ids,key=lambda g:(name_by_id.get(g,''),g)),
}
print('stage static',flush=True)
for m,seq in static_sequences.items():
    for km in key_reprs:add_result(m,key_mode=km,term_seq=seq)

print('static done',len(results),flush=True)
# 2. GAF one-phase and two-phase models. Limit full cross product sensibly.
for fname in ('exact_all_accessions','exact_historical_mapped'):
    ff=filters[fname]
    fr=[r for r in rows if ff(r)]
    for oname in ('source','accession_GO','date_accession_GO'):
        of=orders[oname]
        ordered=of(fr)
        direct_first=[];seen=set()
        for r in ordered:
            if r['go'] not in seen:seen.add(r['go']);direct_first.append(r['go'])
        # direct-only order for all representations
        if fname in ('exact_all_accessions','exact_historical_mapped') and oname in ('source','accession_GO','date_accession_GO'):
            for km in key_reprs:add_result('gaf_direct_only',fname,oname,'',km,direct_first)
        else:
            add_result('gaf_direct_only',fname,oname,'','GO_ID',direct_first)
        # per-first direct + ancestors; GO_ID all ancestor modes, alternate repr best-common modes
        for aname in ('sorted_asc','dfs_pre_obo','bfs_obo','py2set_dfs_35'):
            af=anc_modes[aname]
            seq=[]
            for g in direct_first:
                seq.append(g); seq.extend(af(g))
            if fname in ('exact_all_accessions','exact_historical_mapped') and oname in ('source','accession_GO','date_accession_GO') and aname in ('sorted_asc','py2set_dfs_35','bfs_obo'):
                kms=('GO_ID','GO_name','MSigDB_standard_name','GO_numeric_int')
            else:
                kms=('GO_ID',)
            for km in kms:
                add_result('first_direct_then_ancestors',fname,oname,aname,km,seq)
        # collect direct dict, then propagate in final Py2 direct-key order
        for km in ('GO_ID','GO_name','MSigDB_standard_name'):
            direct_pred=dict_order_from_term_sequence(direct_first,km)
            for aname in ('sorted_asc','dfs_pre_obo','bfs_obo','py2set_dfs_35','py2set_bfs_35'):
                af=anc_modes[aname];seq=list(direct_first)
                for g in direct_pred:seq.extend(af(g))
                add_result('direct_dict_then_propagate_py2keys',fname,oname,aname,km,seq)


print('gaf models done',len(results),flush=True)
# 6. Table-size / hash-secret sanity: raw selected target under many table masks and representations.
for km in ('GO_ID','GO_name','MSigDB_standard_name','GO_numeric_int'):
    kf=key_reprs[km]
    for bits_n in range(7,20):
        mask=(1<<bits_n)-1
        # Sort by raw hash bucket only, tie by key repr string. Proxy, not collision-aware.
        seq=sorted(target,key=lambda g:((hash_key(kf(g)) & mask),repr(kf(g))))
        sc=score(seq)
        if sc:
            rec={k:v for k,v in sc.items() if k not in ('assigned_observed','predicted')}
            rec.update(model='raw_hash_bucket_proxy',filter='',row_order='',ancestor_order='',key_mode=km,input_terms=121,unique_dictionary_terms=121,mask_bits=bits_n,table_size=1<<bits_n,assigned_observed=sc['assigned_observed'],predicted=sc['predicted'])
            results.append(rec)
...
res.to_csv(OUT/'B104D_deep_column_order_model_grid.csv.gz',index=False,compression='gzip')
...
res[cols].head(250).to_csv(OUT/'B104D_deep_column_order_top250.csv',index=False)
...
with open(OUT/'B104D_deep_column_order_summary.json','w') as f:json.dump(summary,f,indent=2,sort_keys=True,default=str)
...
pd.DataFrame(comp).to_csv(OUT/'B104D_best_model_per_column_comparison.csv',index=False)
