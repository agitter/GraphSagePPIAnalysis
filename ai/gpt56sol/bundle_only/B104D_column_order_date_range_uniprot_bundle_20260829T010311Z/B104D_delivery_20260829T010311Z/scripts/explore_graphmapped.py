...
OUT=ROOT/'analysis_graphmapped'; OUT.mkdir(parents=True,exist_ok=True)
...
for fname in ('exact_graph_mapped',):
    ff=filters[fname]
    fr=[r for r in rows if ff(r)]
    for oname in ('source','accession_GO','date_accession_GO','GeneID_GO'):
        of=orders[oname]
        ordered=of(fr)
        ...
        for aname in ('sorted_asc','dfs_pre_obo','bfs_obo','py2set_dfs_35'):
            ...
            if aname in ('sorted_asc','py2set_dfs_35','bfs_obo'):
...
res.to_csv(OUT/'B104D_graphmapped_column_order_model_grid.csv.gz',index=False,compression='gzip')
...
res[cols].head(250).to_csv(OUT/'B104D_graphmapped_column_order_top250.csv',index=False)
...
with open(OUT/'B104D_graphmapped_column_order_summary.json','w') as f:json.dump(summary,f,indent=2,sort_keys=True,default=str)
...
pd.DataFrame(comp).to_csv(OUT/'B104D_graphmapped_best_model_per_column_comparison.csv',index=False)
